...
/dev/shm/initrd:
	...
	umount /dev/shm/boot_tmp || true
	mount $(DISK_IMAGE) /dev/shm/boot_tmp
	export vmlinuz=$$(ls -1 $$(readlink -f /dev/shm/boot_tmp/boot/vmlinuz)* | sort -V | grep -v '\-6' | tail -1) ;\
	export initrd=$$(readlink -f /dev/shm/boot_tmp/boot/initramfs-$$(echo $$vmlinuz | awk -F'vmlinuz-' '{print $$(NF)}').img) ;\
	echo $$vmlinuz ;\
	echo $$initrd ;\
	cp $$vmlinuz /dev/shm/ ;\
	cp $$initrd /dev/shm/ ;\
	cp $$vmlinuz /dev/shm/vmlinuz ;\
	cp $$initrd /dev/shm/initrd ;\
	ls -l /dev/shm/boot_tmp/boot/ ;\
	ls -l /dev/shm/ ;\
	touch /dev/shm/vmlinuz ;\
	touch /dev/shm/initrd ;\
	umount /dev/shm/boot_tmp ;\
	umount /dev/shm/boot_tmp || true ;\
	umount /dev/shm/boot_tmp || true ;\
	rmdir /dev/shm/boot_tmp

cleanup:
	rm /dev/shm/vmlinuz
	rm /dev/shm/initrd

$(GRUB_ROOT_DIR)/ipxe/git/src/vmlinuz: /dev/shm/initrd
	cp -Lvu /dev/shm/vmlinuz $(GRUB_ROOT_DIR)/ipxe/git/src/vmlinuz
$(GRUB_ROOT_DIR)/ipxe/git/src/initrd: /dev/shm/initrd
	cp -Lvu /dev/shm/initrd $(GRUB_ROOT_DIR)/ipxe/git/src/initrd
...
$(GRUB_ROOT_DIR)/grub.booti386: $(GRUB_ROOT_DIR)/grub.cfg $(GRUB_ROOT_DIR)/grub_template_bios.cfg $(GRUB_ROOT_DIR)/grub_make_bios/build.sh $(GRUB_ROOT_DIR)/grub_make_bios/early_grub.cfg
	cat $(GRUB_ROOT_DIR)/grub_template_bios.cfg | grep -v '#' | sed "s/PXE_TEMP_IP/${PXE_TEMP_IP}/g" | sed "s/PXE_TFTP/${PXE_TFTP}/g" | sed "s/DISK/${DISK}/g"  > $(GRUB_ROOT_DIR)/grub_make_bios/grub.cfg
	#cat $(GRUB_ROOT_DIR)/grub_template_menu.cfg | grep -v '#' | sed "s/PXE_TEMP_IP/${PXE_TEMP_IP}/g" | sed "s/PXE_TFTP/${PXE_TFTP}/g" | sed "s/DISK/${DISK}/g"  >> $(GRUB_ROOT_DIR)/grub_make_bios/grub.cfg
	#cat $(GRUB_ROOT_DIR)/grub_template_menu.cfg | grep -v '#' | sed "s/PXE_TEMP_IP/${PXE_TEMP_IP}/g" | sed "s/PXE_TFTP/${PXE_TFTP}/g" | sed "s/DISK/${DISK}/g"  > $(GRUB_ROOT_DIR)/grub_make_bios/grub_menu.cfg
